...
router.get('/', passport.authenticate('oauth-bearer', { session: false }), (req, res) => {
  ...
  query.exec((err, docs) => {
    if (err || docs.length === 0) return res.status(404).json('No students found')
    res.json({ students: docs })
  })
})

router.post('/add', passport.authenticate('oauth-bearer', { session: false }), (req, res) => {
...
router.post('/get_profiles', passport.authenticate('oauth-bearer', { session: false }), async (req, res) => {
  let requests = {
    requests: []
  }
  ...
  if (req.body.students.length > 0) {
    ...
    getAccessTokenOnBehalfOf(req.headers.authorization.substring(7), 'https://graph.microsoft.com/user.read', (accessToken) => {
      axios.post('https://graph.microsoft.com/v1.0/$batch', requests, setupHeader(accessToken))
        .then(resp => {
          ...
          for (let response of resp.data.responses) {
            if (response.status === 200) {
              data.push(_pick(response.body, ['id', 'userPrincipalName', 'displayName']))
            } else {
...
router.post('/assign-test', passport.authenticate('oauth-bearer', { session: false }), async (req, res) => {
  if (req.body.students && req.body.students.length > 0) {
    ...
    let existingEmails = await Student.find({ email: { $in: students.map(student => { return student.email }) } }).select({ email: 1, displayName: 1, _id: 0 })
      .catch(err => {
      ...
      async (accessToken) => {
        ...
        for (let studentData of students) {
          if (studentData.status === 'unknown') {
            ...
            let profileData = await axios.get(`https://graph.microsoft.com/v1.0/users/${studentData.email}`, setupHeader(accessToken))
              .catch(err => {
            ...
            if (profileData) {
              ...
              studentData.firstName = profileData.data.givenName.charAt(0).toUpperCase() + profileData.data.givenName.substring(1)
              studentData.lastName = profileData.data.surname.charAt(0).toUpperCase() + profileData.data.surname.substring(1)
            ...
            if (studentData.status === 'assigned' || studentData.status === 'already_assigned') {
              new Student({
