...
router.post('/assign', passport.authenticate('oauth-bearer', { session: false }), async (req, res) => {
  ...
  if (req.body.supervisors && req.body.supervisors.length > 0) {
    ...
    let existingEmails = await Supervisor.find({ email: { $in: supervisors.map(supervisor => { return supervisor.email }) } }).select({ email: 1, displayName: 1, _id: 0 })
      .catch(err => {
    ...
    for (let supervisor of existingEmails) {
      var elemIndex = supervisors.map((spvsr) => spvsr.email).indexOf(supervisor.email)
      if (elemIndex > -1) {
      ...
      async (accessToken) => {
        ...
        for (let supervisorData of supervisors) {
          if (supervisorData.status === 'unknown') {
            ...
            let profileData = await axios.get(`https://graph.microsoft.com/v1.0/users/${supervisorData.email}`, setupHeader(accessToken))
              .catch(err => {
            ...
            if (profileData) {
              ...
              supervisorData.firstName = profileData.data.givenName.charAt(0).toUpperCase() + profileData.data.givenName.substring(1)
              supervisorData.lastName = profileData.data.surname.charAt(0).toUpperCase() + profileData.data.surname.substring(1)
            ...
            if (supervisorData.status === 'assigned' || supervisorData.status === 'already_assigned') {
              new Supervisor({
